...
class JoinRoomMixin:
    ...
    def join(self, request, *args, **kwargs):
        ...
        """
        instance = self.get_queryset().get(code=self.kwargs[self.lookup_field_code]) # get the room, user will join in if exist
        serializer = self.get_serializer(instance=instance, data=request.data)
        serializer.is_valid(raise_exception=True)
        user = get_object_or_404(User, username=request.COOKIES.get('user').username)
